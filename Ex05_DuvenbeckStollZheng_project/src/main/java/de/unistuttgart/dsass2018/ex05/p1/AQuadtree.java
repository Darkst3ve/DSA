...
import de.unistuttgart.dsass2018.ex05.p1.Rect;
...
public abstract class AQuadtree<T extends ATreeObject> {
	...
	/**
	 * Quadtree-Nodes that cover a sub-area of the current node
	 */
	...
	/**
	 * The maximum number of elements in a leaf before a leaf node is being
	 * split
	 */
	protected int maxElementNumberInLeafs;

	/**
	 * Elements that are leaf nodes
	 */
	...
	@Override
	/**
	 * Checks if two Quadtrees have the same structure and the same elements
	 * stored
	 */
	public boolean equals(Object obj) {
		...
		try {
			...
			// if the cast fails, obj is not a AQuadtree
			return false;
		}

		// two trees are equal if their subtrees are equal
		if (this.getLeafElements() == null && b.getLeafElements() == null) {
			...
			if ((this.getLeafElements() == null && b.getLeafElements() != null)
					|| (this.getLeafElements() != null && b.getLeafElements() == null)) {
				return false;
			}
			...
			// two trees are equal if their leaf nodes are equal
	 ...
	 * 
	 * @param resultList:
	 *            List that is used to store the elements that are contained in
	 *            the selectedArea
	 * @param selecedArea:
	 *            The method is looking for elements that are located in the
	 *            selectedArea
	 */
	 ...
	 * 
	 * @param writer:
	 *            Specifies a reference to a FileWriter
	 * @param sep:
	...
	private void saveNodesRecursive(FileWriter writer, String sep) {
		...
		try {
			if (this.leafElements == null) {
				...
				writer.write("O;" + this.area.x + sep + this.area.y + sep + this.area.width + sep + this.area.height
						+ sep + this.maxElementNumberInLeafs);
				...
				writer.write("B;" + this.area.x + sep + this.area.y + sep + this.area.width + sep + this.area.height
						+ sep + this.maxElementNumberInLeafs + sep);
				if (this.leafElements == null)
					writer.write("0");
				else
	...
	/**
	 * Prints the size of the leaf elements to the console. The tree traversal
	 * is inorder.
	 */
	public void inorder() {
